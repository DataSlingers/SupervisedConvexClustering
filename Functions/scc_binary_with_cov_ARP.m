function [U_output,Z_output,beta_output] = scc_binary_with_cov_ARP(X,Y,T,w,target_K)
%%% Matrix Update
%%% Z: n * p_cov
...
beta_output = zeros(p_cov,MAX_ITER);
...
while norm(Z_output(:,:,k)) ~= 0
    ...
    for m = 1:MAX_ITER_INNER
        ...
        % V update
        V = (1/4* alpha_Y * V + alpha_Y * Y - alpha_Y * exp(V+(T * beta)')./(1+exp(V+(T * beta)'))+ (Z2+Lambda2) * D') * M2;

        % beta update
        beta = beta - inv_cov * ( T' * ( -Y' + exp(V'+T * beta)./(1+exp(V'+ T * beta)) ) );
...
if isempty(class_id_final)
    class_id_final = class_id;
end

U_output = U_output(:,:,1:k);
Z_output = Z_output(:,:,1:k);
beta_output = beta_output(:,1:k);
