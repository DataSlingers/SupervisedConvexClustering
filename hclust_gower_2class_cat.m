function [ conca_hclust_ri, agg_hclust_ri ] = hclust_gower_2class_cat( X,Y,K,truth )
...
for r = 1:trials
    D = squareform(gower_2data_with_cat(X,Y));
...
hclust_ri_ind = zeros(1,trials);
for r = 1:trials
    D = squareform(agg_gower_2data_with_cat(X,Y));
    ZZ = linkage(D,'complete');
    % Z = linkage(X','average','cityblock');
    idx3 = cluster(ZZ,'maxclust',K);
    hclust_ri_ind(r) = rand_index(idx3',truth,'adjusted');
